...
namespace Microsoft.Extensions.Configuration.AzureAppConfiguration.AzureKeyVault
{
    internal class AzureKeyVaultKeyValueAdapter : IKeyValueAdapter
    {
        ...
        public async Task<IEnumerable<KeyValuePair<string, string>>> ProcessKeyValue(ConfigurationSetting setting, CancellationToken cancellationToken)
        {
            ...
            try
            {
                secret = await _secretProvider.GetSecretValue(secretUri, setting, cancellationToken).ConfigureAwait(false);
            }
        ...
        public void InvalidateCache(ConfigurationSetting setting = null)
        {
            ...
            {
                _secretProvider.RemoveExpiredSecretFromCache(setting.Key, setting.Label);
            }
        }
