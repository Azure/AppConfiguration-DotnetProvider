...
namespace Microsoft.Extensions.Configuration.AzureAppConfiguration
{
    internal class AzureAppConfigurationProvider : ConfigurationProvider, IConfigurationRefresher, IDisposable
    {
        ...
        private Dictionary<SettingSelector, IEnumerable<MatchConditions>> _watchedCollections = new Dictionary<SettingSelector, IEnumerable<MatchConditions>>();
        ...
        public async Task RefreshAsync(CancellationToken cancellationToken)
        {
            ...
            if (Interlocked.Exchange(ref _networkOperationsInProgress, 1) == 0)
            {
                try
                {
                    ...
                    Dictionary<SettingSelector, IEnumerable<MatchConditions>> watchedCollections = null;
                    ...
                    await ExecuteWithFailOverPolicyAsync(clients, async (client) =>
                        {
                            ...
                            bool hasWatchedCollectionsChanged = false;
                            ...
                            if (!refreshAll)
                            {
                                foreach (KeyValueWatcher multiKeyWatcher in refreshableMultiKeyWatchers)
                                {
                                    ...
                                    if (_watchedCollections.TryGetValue(watchedSettingSelector, out IEnumerable<MatchConditions> matchConditions))
                                    {
                                        await TracingUtils.CallWithRequestTracing(_requestTracingEnabled, RequestType.Watch, _requestTracingOptions,
                                            async () => hasWatchedCollectionsChanged = await client.HasAnyKeyValueChanged(watchedSettingSelector, matchConditions, cancellationToken).ConfigureAwait(false)).ConfigureAwait(false);
                                    }
                            ...
                            else if (hasWatchedCollectionsChanged && _options.RegisterAllEnabled)
                            {
                                ...
                                (data, watchedCollections) = await LoadSelectedKeyValues(client, cancellationToken).ConfigureAwait(false);
                                watchedSettings = await LoadKeyValuesRegisteredForRefresh(client, data, cancellationToken).ConfigureAwait(false);
                                watchedCollections = UpdateWatchedKeyValueCollections(watchedCollections);
                    ...
                    if (!refreshAll)
                    {
                        ...
                        {
                            SettingSelector changeSelector = new SettingSelector()
                            {
                                KeyFilter = change.Key,
                                LabelFilter = change.Label
                            };

                            if (change.ChangeType == KeyValueChangeType.Modified)
                            {
                                ConfigurationSetting setting = change.Current;
                                watchedSettings[changeSelector] = new List<MatchConditions>() { new MatchConditions() { IfNoneMatch = setting.ETag } };
                            ...
                            {
                                _mappedData.Remove(change.Key);
                                watchedSettings.Remove(changeSelector);
                            }
        ...
        private async Task InitializeAsync(IEnumerable<ConfigurationClient> clients, CancellationToken cancellationToken = default)
        {
            ...
            await ExecuteWithFailOverPolicyAsync(
                ...
                async (client) =>
                {
                    (data, watchedSettings) = await LoadSelectedKeyValues(
                        client,
                        cancellationToken)
                        .ConfigureAwait(false);
        ...
        private async Task<(Dictionary<string, ConfigurationSetting>, Dictionary<SettingSelector, IEnumerable<MatchConditions>>)> LoadSelectedKeyValues(ConfigurationClient client, CancellationToken cancellationToken)
        {
            ...
            return (serverData, watchedSettings);
        }

        private async Task<Dictionary<SettingSelector, IEnumerable<MatchConditions>>> LoadKeyValuesRegisteredForRefresh(ConfigurationClient client, IDictionary<string, ConfigurationSetting> existingSettings, Dictionary<SettingSelector, IEnumerable<MatchConditions>> existingWatchedSettings, CancellationToken cancellationToken)
        {
            Dictionary<SettingSelector, IEnumerable<MatchConditions>> watchedSettings = new Dictionary<SettingSelector, IEnumerable<MatchConditions>>(existingWatchedSettings);
