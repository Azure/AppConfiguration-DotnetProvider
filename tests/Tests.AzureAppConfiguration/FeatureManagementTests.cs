...
namespace Tests.AzureAppConfiguration
{
    ...
    public class FeatureManagementTests
    {
        ...
        [Fact]
        public void FeatureFlagRetainPrecedenceWithChange()
        {
            IConfigurationRefresher refresher = null;
            var featureFlags = _featureFlagCollection2;
            ...
            var config = new ConfigurationBuilder()
                .AddAzureAppConfiguration(options =>
                {
                    ...
                    options.UseFeatureFlags(ffOptions =>
                        {
                            ffOptions.Select(KeyFilter.Any, "label1");
                            ffOptions.Select(KeyFilter.Any, "label2");
                            ffOptions.Select(KeyFilter.Any, "label3");
                            ffOptions.CacheExpirationInterval = CacheExpirationTime;
                        });
                    refresher = options.GetRefresher();
                })
                .Build();

            Assert.Equal("SuperUsers", config["FeatureManagement:Feature:EnabledFor:0:Name"]);
        ...
        [Fact]
        public void FeatureFlagRetainPrecedenceWithDelete()
        {
            IConfigurationRefresher refresher = null;
            var featureFlags = _featureFlagCollection2;
            ...
            featureFlags.RemoveAt(2);

            Thread.Sleep(CacheExpirationTime);
            refresher.TryRefreshAsync().Wait();
            Assert.Equal("Users", config["FeatureManagement:Feature:EnabledFor:0:Name"]);
        }
